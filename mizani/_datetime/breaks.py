...
class Helper:
    ...
    def intervals_sec(self) -> Sequence[float]:
        ...
        """
        return (
            *_to_seconds(self.microseconds, USEC),
            *self.seconds,
            *_to_seconds(self.minutes, MIN),
            *_to_seconds(self.hours, HOUR),
            *_to_seconds(self.days, DAY),
            *_to_seconds(self.weeks, WEEK),
            *_to_seconds(self.months, MONTH),
            *_to_seconds(self.years, YEAR),
        )
    ...
    def rounders(self) -> Sequence[DateTimeRounder]:
        ...
        lookup_unit: dict[int | float, DateTimeRounder] = {
            ...
            1: rounding.minutes,
            2 * MIN: rounding.hours,
            3 * HOUR: rounding.days,
            1 * WEEK: rounding.weeks,
            MONTH // 2: rounding.months,
            3 * MONTH: rounding.years,
            2 * YEAR: rounding.decades,
            20 * YEAR: rounding.centurys,
        }
...
def by_width(
    ...
    return [b + offset for b in trim_breaks(list(r), limits)]
