...
"""
# Last sync version: __version__ = '5.0.3'
# Ref: https://github.com/hsluv/hsluv-python/blob/9c494db/hsluv.py
...
if TYPE_CHECKING:
    ...
    Line: TypeAlias = dict[Literal["slope", "intercept"], float]
...
def _max_chroma_for_lh(l: float, h: float) -> float:
    ...
    lengths = [
        _length_of_ray_until_intersect(hrad, bound) for bound in _get_bounds(l)
    ]
...
def xyz_to_rgb(_hx_tuple: Triplet) -> Triplet:
    return (
...
def rgb_to_xyz(_hx_tuple: Triplet) -> Triplet:
    rgbl = (
...
def xyz_to_luv(_hx_tuple: Triplet) -> Triplet:
    x = float(_hx_tuple[0])
    y = float(_hx_tuple[1])
    z = float(_hx_tuple[2])
    l = _y_to_l(y)
...
def luv_to_xyz(_hx_tuple: Triplet) -> Triplet:
    l = float(_hx_tuple[0])
    u = float(_hx_tuple[1])
    v = float(_hx_tuple[2])
    if l == 0:
...
def luv_to_lch(_hx_tuple: Triplet) -> Triplet:
    l = float(_hx_tuple[0])
    u = float(_hx_tuple[1])
    v = float(_hx_tuple[2])
    c = _math.hypot(u, v)
...
def lch_to_luv(_hx_tuple: Triplet) -> Triplet:
    l = float(_hx_tuple[0])
    c = float(_hx_tuple[1])
    h = float(_hx_tuple[2])
    hrad = _math.radians(h)
...
def hsluv_to_lch(_hx_tuple: Triplet) -> Triplet:
    h = float(_hx_tuple[0])
    s = float(_hx_tuple[1])
    l = float(_hx_tuple[2])
    if l > 100 - 1e-7:
...
def hpluv_to_lch(_hx_tuple: Triplet) -> Triplet:
    h = float(_hx_tuple[0])
    s = float(_hx_tuple[1])
    l = float(_hx_tuple[2])
    if l > 100 - 1e-7:
...
def lch_to_rgb(_hx_tuple: Triplet) -> RGBColor:
    return xyz_to_rgb(luv_to_xyz(lch_to_luv(_hx_tuple)))


def rgb_to_lch(_hx_tuple: RGBColor) -> Triplet:
